...
public class CellularMatrix {
    ...
    // public List<Boid> boids = new ArrayList<>();
    ...
    // public CellularMatrix(int width, int height, int pixelSizeModifier, World world) {
        ...
        this.pixelBuffer = new PixelBuffer(innerArraySize, outerArraySize);
    ...
    private List<List<Chunk>> generateChunks() {
        ...
        for (int r = 0; r < rows; r++) {
            ...
            for (int c = 0; c < columns; c++) {
                ...
                newChunk.setTopLeft(new Vector3().x(Math.min(xPos, innerArraySize)).y(Math.min(yPos, outerArraySize)).z(0));
                newChunk.setBottomRight(new Vector3().x(Math.min(xPos + Chunk.size, innerArraySize)).y(Math.min(yPos + Chunk.size, outerArraySize)).z(0));
            }
        }
        return chunks;
    }
    ...
    // public void drawAll(Raylib.Image image) {
    //     drawElements(image);
    // }

    // public void drawElements(Raylib.Image image) {
    //     for (int y = 0; y < outerArraySize; y++) {
    //         List<Element> row = getRow(y);
    //         for (int x = 0; x < row.size(); x++) {
    //             Element element = row.get(x);
    //             ImageDrawPixel(image, x, y, element.color);
    //         }
    //     }
    // }
...
//     }
    ...
    // public void drawProvidedColumn(int colIndex, Raylib.Image image) {
    //     for (int y = 0; y < outerArraySize; y++) {
    //         List<Element> row = getRow(y);
    //         for (int x : shuffledXIndexesForThreads.get(colIndex)) {
    //             try {
    //                 Element element = row.get(calculateIndexWithOffset(x));
    //                 ImageDrawPixel(image, calculateIndexWithOffset(x), y, element.color);
    //             } catch (Exception e) {
    //                 e.printStackTrace();
    //             }
    //         }
    ...
    public void particalizeByPixelWithBrush(int x, int y, int brushSize, InputManager.BRUSHTYPE brushtype) {
        ...
        particalizeByMatrixWithBrush(createFunctionInput(matrixX, matrixY, null, brushSize, new Vector3().x(0).y(-124).z(0), brushtype));
    }
...
//    }
    ...
    // public void updatePixels(Raylib.Image image) {
    //     for (int y = 0; y < outerArraySize; y++) {
    //         List<Element> row = getRow(y);
    //         for (int x = 0; x < row.size(); x++) {
    //             Element element = row.get(x);
