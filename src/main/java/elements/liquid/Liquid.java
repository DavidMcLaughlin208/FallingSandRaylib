...
public abstract class Liquid extends Element {
    ...
    public void step(CellularMatrix matrix) {
        ...
        VectorUtils.add(vel, CellularAutomaton.gravity);
        if (isFreeFalling) vel.x((float) (vel.x() * 0.8));
        ...
        int smallerCount;
        Vector3 formerLocation = new Vector3().x(getMatrixX()).y(getMatrixY()).z(0);
...
//            }
            Vector3 normalizedVel = VectorUtils.getNormalized(vel);
            ...
            if (adjacentNeighbor != null && adjacentNeighbor != diagonalNeighbor) {
                ...
                if (stoppedAdjacently) vel.x(vel.x() * -1);
    ...
    private boolean iterateToAdditional(CellularMatrix matrix, int startingX, int startingY, int distance, Vector3 lastValid) {
        ...
        Vector3 lastValidLocation = VectorUtils.copy(lastValid);
